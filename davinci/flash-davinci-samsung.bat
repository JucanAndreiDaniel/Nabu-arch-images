@echo off
REM Flash Arch (davinci, Samsung panel): prebuilt U-Boot -> boot,
REM nested GPT image (FAT ESP + ext4 root) -> userdata.
...
if not exist "%~dp0images\userdata-nested.img" echo Missing userdata-nested.img && exit /B 1
...
fastboot %* flash userdata %~dp0images\userdata-nested.img || exit /B 1
