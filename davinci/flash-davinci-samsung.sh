#!/usr/bin/env bash
# Flash Arch (davinci, Samsung panel) : prebuilt U-Boot -> boot, rootfs -> userdata.
# Requires: unlocked bootloader, fastboot mode, dtbo backup already taken.
set -euo pipefail

DIR="$(cd "$(dirname "$0")" && pwd)"
BOOT_IMG="$DIR/images/boot-davinci-samsung.img"
ROOTFS_IMG="$DIR/images/rootfs-davinci-samsung.img"
...
fastboot "$@" flash userdata "$ROOTFS_IMG" || { echo "Flash rootfs error"; exit 1; }
